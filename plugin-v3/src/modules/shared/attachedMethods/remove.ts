...
export function createRemoveMethod(plugin: PsdToPhaserPlugin) {
  return function remove(this: Phaser.GameObjects.GameObject | Phaser.GameObjects.Group, pathOrOptions?: string | { depth?: number }, options: { depth?: number } = {}): boolean {
    ...
    if (typeof pathOrOptions === 'string') {
      ...
      depth = pathOrOptions.depth !== undefined ? pathOrOptions.depth : Infinity;
    }
    ...
    const findAndRemove = (obj: Phaser.GameObjects.GameObject | Phaser.GameObjects.Group, pathParts: string[]): boolean => {
      ...
      if (obj instanceof Phaser.GameObjects.Group) {
        ...
        const targetChild = obj.getChildren().find(child => child.name === targetName);
