...
export function LazyLoadCamera(plugin: PsdToPhaserPlugin, camera: Phaser.Cameras.Scene2D.Camera, psdKey: string, options: LazyLoadOptions = {}) {
  ...
  let lazyLoadObjects: Array<{ boundary: Phaser.GameObjects.Rectangle, data: any }> = [];
...
let intervalId: number | null = null;
  ...
  function createLazyLoadObjects() {
    ...
    lazyLoadObjects = [
      ...
      ...(psdData.lazyLoad.tiles || []).flatMap(tileset => tileToLazyObjects(tileset).map(createObject))
    ];
    ...
    if (options.debug?.label) {
      lazyLoadObjects.forEach(obj => {
        const text = scene.add.text(obj.boundary.x, obj.boundary.y - 20, obj.data.name, {
          fontSize: '12px',
          color: '#00ffff',
  ...
  function checkVisibility() {
    const objectsToLoad = lazyLoadObjects.filter(({ boundary, data }) => 
      !data.loaded && 
      !objectsBeingLoaded.has(getObjectKey(data)) &&
      Phaser.Geom.Intersects.RectangleToRectangle(boundary.getBounds(), lazyLoadBounds)
    );
  ...
  const objectType = data.category || data.type;
  if (objectType === 'sprite') {
    loadSprites(scene, [data], psdData.basePath, () => onObjectLoaded(data), plugin.isDebugEnabled('console'));
  } else if (objectType === 'tile' || objectType === 'tileset') {
    loadSingleTile(
      scene,
      data,
      psdData.basePath,
      psdData.original.tile_slice_size,
      () => onObjectLoaded(data),
      plugin.isDebugEnabled('console')
    );
  } else {
    console.error('Unknown object type:', objectType, data);
  ...
  function onObjectLoaded(data: any) {
    ...
    const tileKey = `${data.tilesetName}_tile_${data.col}_${data.row}`;
    const placedTile = placeSingleTile(
      scene,
      {
        x: data.x,
        y: data.y,
    ...
    if (placedTile) {
      console.log(`Lazy loaded tile placed: ${tileKey}`);
    } else {
      console.warn(`Failed to place lazy loaded tile: ${tileKey}`);
    }
    }

    const progress = lazyLoadObjects.filter(({ data }) => data.loaded).length / lazyLoadObjects.length;
    const remainingObjects = Array.from(objectsBeingLoaded);
    scene.events.emit('lazyLoadProgress', progress, remainingObjects);
    console.log(`LazyLoad: Progress ${progress.toFixed(2)}, Remaining:`, remainingObjects);
