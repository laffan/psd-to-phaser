...
Several sprite methods can be called on a placed group. This just applies the method to each sprite individually. Currently, the list of supported methods (somewhat arbitrarily) includes : 'setActive', 'setAlpha', 'setAngle', 'setBlendMode', 'setDepth', 'setDisplaySize', 'setFlip', 'setOrigin', 'setPosition', 'setRotation', 'setScale', 'setScrollFactor', 'setSize', 'setTint', 'setVisible', 'setX', 'setY', 'setZ'.

> **Note (Phaser 4):** `setMask` and `setPipeline` have been removed. In Phaser 4, masks are applied via the [Filter system](#layer-masks) and pipelines have been replaced by the RenderNode architecture.
...
For the full migration guide, see [MIGRATION-3-TO-4.md](./MIGRATION-3-TO-4.md).
