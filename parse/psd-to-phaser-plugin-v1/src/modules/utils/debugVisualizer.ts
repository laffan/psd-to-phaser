...
export function createDebugShape(
  scene: Phaser.Scene,
  type: string,
  x: number,
  y: number,
  ...
  if (debugOptions.shape) {
    switch (type) {
      case "point":
        ...
        break;
      case "sprite":
        debugShape = scene.add.rectangle(
          x,
          y,
          options.width || 10,
          options.height || 10,
        ...
        break;
      case "zone":
        debugShape = createZoneDebugShape(
          scene,
          options.shape,
  ...
  if (debugOptions.label && options.name) {
    const debugText = scene.add.text(x, y, options.name, {
      fontSize: "16px",
      color: "#ffffff",
    });
