...
export default function pointsModule(plugin) {
    return {
        place(scene, pointPath, psdKey, options = {}) {
            ...
            const rootPoint = psdData.points.find(p => pointPath.startsWith(p.name));
            if (!rootPoint) {
                console.warn(`Point '${pointPath}' not found in PSD data for key '${psdKey}'.`);
                return null;
            }
        ...
        placePoint(scene, point, options = {}) {
            ...
            const debugGraphics = point.addDebugVisualization(scene, 'point', plugin, options);
            ...
            if (point.children) {
                result.children = point.children.map(child => this.placePoint(scene, child, options));
            }
