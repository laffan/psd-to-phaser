...
Several sprite methods can be called on a placed group. This just applies the method to each sprite individually. Currently, the list of supported methods includes : 'setAlpha', 'setAngle', 'setBlendMode', 'setDepth', 'setDisplaySize', 'setFlip', 'setOrigin', 'setPosition', 'setRotation', 'setScale', 'setScrollFactor', 'setSize', 'setTint', 'setVisible', 'setX', 'setY', 'setZ'.

> **Phaser 4 note:** `setMask` and `setPipeline` have been removed from the default methods. In Phaser 4, masks are applied via the [Filter system](#layer-masks) and pipelines have been replaced by the RenderNode architecture.
...
In the PSD, a group layer with a mask will export with `mask: true`, position properties (`maskX`, `maskY`), and a `maskPath` pointing to the mask image. The mask uses Phaser 4's Filter system (`filters.internal.addMask()`) - white areas are visible, black areas are hidden.
