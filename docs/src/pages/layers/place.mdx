...
Several sprite methods can be called on a placed group. This just applies the method to each sprite individually. Currently, the list of supported methods (somewhat arbitrarily) includes : 'setAlpha', 'setAngle', 'setBlendMode', 'setDepth', 'setDisplaySize', 'setFlip', 'setLighting', 'setMask', 'setOrigin', 'setPosition', 'setRotation', 'setScale', 'setScrollFactor', 'setSize', 'setTint', 'setTintMode', 'setVisible', 'setX', 'setY', 'setZ'.
...
In the demo above, notice how the moon and mountains move independently while the circular mask stays fixed. This is because masks are positioned in global space - sprites move "underneath" the mask.

In the PSD, a group layer with a mask will export with `mask: true`, position properties (`maskX`, `maskY`), and a `maskPath` pointing to the mask image. White areas of the mask are visible, black areas are hidden.

Masks are built on Phaser 4's [Filter system](https://docs.phaser.io/phaser/concepts/filters), which is WebGL only - under the Canvas renderer masked layers render unmasked. Every filtered object also renders through its own framebuffer, so a mask on a large group is not free: mask the smallest set of layers you can.
